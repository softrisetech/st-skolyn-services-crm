...
class FollowUpTypeView(APIView):
    ...
    def get(self, request, pk=None):
        ...
        if pk:
            follow_up_type = queryset.filter(id=pk).first()
            if follow_up_type:
    ...
    def put(self, request, pk=None):
        ...
        business_id = data.get('auth_business_id')
        try:
            follow_up_type = FollowUpType.objects.get(id=pk, business_id=business_id)
    ...
    def delete(self, request, pk=None):
        ...
        try:
            follow_up_type = FollowUpType.objects.get(id=pk, business_id=business_id)
            child_references = [
                {'model': FollowUp, 'foreign_key': 'type_id'}
            ]
...
def change_follow_up_type_status(request, pk=None):
    data = request.data.copy()
    business_id = data.get('auth_business_id')
    try:
        follow_up_type = FollowUpType.objects.get(id=pk, business_id=business_id)
    ...
    data['business_id'] = business_id
    serializer = FollowUpTypeSerializer(instance=follow_up_type, data=data, partial=True)
    if serializer.is_valid():
        serializer.save()
        return success_response('record_updated', status.HTTP_200_OK, serializer.data)
    return error_response('record_update_failed', status.HTTP_422_UNPROCESSABLE_ENTITY, serializer.errors)
