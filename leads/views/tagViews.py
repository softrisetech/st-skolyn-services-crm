...
@api_view(['POST'])
@access_control_middleware
def get_lead_tag(request, pk=None):
    data = request.data
    timezone = data.get("auth_timezone")
    business_id = data.get('auth_business_id')
    tag = __queryset(business_id).filter(id=pk).first()
    if not tag:
        return error_response('tag_not_found', status.HTTP_404_NOT_FOUND)
...
@api_view(['POST'])
@access_control_middleware
def update_lead_tag(request, pk=None):
    data = request.data.copy()
    business_id = data.get('auth_business_id')
    tag = __queryset(business_id).filter(id=pk).first()
    if not tag:
...
@api_view(['POST'])
@access_control_middleware
def delete_lead_tag(request, pk=None):
