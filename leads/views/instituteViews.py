...
from ..models import Institute, PreRequisite
...
def __queryset(business_id):
    return Institute.objects.filter(business_id=business_id)
...
def get_lead_institutes(request):
    data = request.data
    ...
    serialized_data = InstituteSerializer(paginated_queryset, many=True).data
...
@api_view(['POST'])
@access_control_middleware
def get_lead_institute(request, pk=None):
    data = request.data
    timezone = data.get("auth_timezone")
    business_id = data.get('auth_business_id')
    institute = __queryset(business_id).filter(id=pk).first()
    if not institute:
...
@api_view(['POST'])
@access_control_middleware
def store_lead_institute(request):
...
@api_view(['POST'])
@access_control_middleware
def update_lead_institute(request, pk=None):
    data = request.data.copy()
    business_id = data.get('auth_business_id')
    institute = __queryset(business_id).filter(id=pk).first()
    if not institute:
    ...
    serializer = InstituteSerializer(instance=institute, data=data, partial=False)
    if not serializer.is_valid():
...
@api_view(['POST'])
@access_control_middleware
def delete_lead_institute(request, pk=None):
    ...
    child_references = [
        {'model': PreRequisite, 'foreign_key': 'institute_id'}
    ]
    has_refs = has_active_child_references(child_references, pk, business_id)
    if has_refs:
        return error_response("related_institute_record_found_on_deletion", status.HTTP_400_BAD_REQUEST)
