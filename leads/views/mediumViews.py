...
from ..models import Medium, Lead
...
class MediumView(APIView):
    ...
    def get_queryset(self, business_id):
        ...
        return Medium.objects.filter(business_id=business_id)
    ...
    def get(self, request, pk=None):
        ...
        if pk:
            medium = queryset.filter(id=pk).first()
            if medium:
                serialized_data = MediumSerializer(medium).data
        ...
        serialized_data = MediumSerializer(paginated_queryset, many=True).data
    ...
    def post(self, request):
        ...
        serializer = MediumSerializer(data=data)
    ...
    def put(self, request, pk=None):
        ...
        business_id = data.get('auth_business_id')
        try:
            medium = Medium.objects.get(id=pk, business_id=business_id)
        except Medium.DoesNotExist:
            return error_response('record_not_found', status.HTTP_404_NOT_FOUND)

        data['business_id'] = business_id
        serializer = MediumSerializer(instance=medium, data=data, partial=False)
        if serializer.is_valid():
            serializer.save()
    ...
    def delete(self, request, pk=None):
        ...
        try:
            medium = Medium.objects.get(id=pk, business_id=business_id)
            child_references = [
                {'model': Lead, 'foreign_key': 'medium_id'}
            ]
...
def change_medium_status(request, pk=None):
    data = request.data.copy()
    business_id = data.get('auth_business_id')
    try:
        medium = Medium.objects.get(id=pk, business_id=business_id)
    ...
    data['business_id'] = business_id
    serializer = MediumSerializer(instance=medium, data=data, partial=True)
    if serializer.is_valid():
        serializer.save()
        return success_response('record_updated', status.HTTP_200_OK, serializer.data)
    return error_response('record_update_failed', status.HTTP_422_UNPROCESSABLE_ENTITY, serializer.errors)
