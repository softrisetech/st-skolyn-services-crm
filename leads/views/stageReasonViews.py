...
from ..models import StageReason, StageReasonEntry
from ..serializers import StageReasonSerializer
...
from ..utils.filters import filter_by_active, filter_by_sort_order, filter_by_stages


def __queryset(business_id):
    return StageReason.objects.filter(business_id=business_id)
...
def get_stage_reasons(request):
    ...
    serialized_data = StageReasonSerializer(paginated_queryset, many=True).data
...
@api_view(['POST'])
@access_control_middleware
def get_stage_reason(request, pk=None):
    data = request.data
    timezone = data.get("auth_timezone")
    business_id = data.get('auth_business_id')
    stage_reason = __queryset(business_id).filter(id=pk).first()
    if not stage_reason:
        return error_response('stage_reason_not_found', status.HTTP_404_NOT_FOUND)

    serialized_data = StageReasonSerializer(stage_reason).data
...
@api_view(['POST'])
@access_control_middleware
def store_stage_reason(request):
    data = request.data.copy()
    data['business_id'] = data.get('auth_business_id')
    serializer = StageReasonSerializer(data=data)
...
@api_view(['POST'])
@access_control_middleware
def update_stage_reason(request, pk=None):
    data = request.data.copy()
    business_id = data.get('auth_business_id')
    stage_reason = __queryset(business_id).filter(id=pk).first()
    if not stage_reason:
    ...
    serializer = StageReasonSerializer(instance=stage_reason, data=data, partial=False)
    if not serializer.is_valid():
...
@api_view(['POST'])
@access_control_middleware
def delete_stage_reason(request, pk=None):
    business_id = request.data.get('auth_business_id')
    stage_reason = __queryset(business_id).filter(id=pk).first()
    if not stage_reason:
        return error_response('stage_reason_not_found', status.HTTP_404_NOT_FOUND)
    child_references = [
        {'model': StageReasonEntry, 'foreign_key': 'stage_reason_id'}
    ]
    has_refs = has_active_child_references(child_references, pk, business_id)
    if has_refs:
        return error_response("related_stage_reason_record_found_on_deletion", status.HTTP_400_BAD_REQUEST)
